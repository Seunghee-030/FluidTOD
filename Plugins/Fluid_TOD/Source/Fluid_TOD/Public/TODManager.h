...
public:
    ...
    float ValueChangeTolerance = 0.0f;
    ...
    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "TOD|Performance",
        meta = (ToolTip = "Re-register the sun/moon light with the atmosphere when its lit state crosses zero intensity. "))
    bool bRefreshAtmosphereLightOnStateChange = true;

    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "TOD|Performance",
        meta = (ToolTip = "Compatibility fallback. Marks the sun/moon light render state dirty on every TOD update, "))
    bool bForceAtmosphereLightRefreshEveryFrame = false;
