...
void ATODManager::Tick(float DeltaSeconds)
{
	...
	// 0/24 이음매에서 강제 재적용을 하지 않는다.
	// bHasAppliedTODSettings를 끄면 UpdateTOD에서 bForceApply가 참이 되고,
	// 그게 라이트 프록시 재생성(MarkRenderStateDirty)까지 끌고 들어간다.
	// 하루에 한 프레임만 대기 광원이 재등록되면서 하늘이 튄다.
	// 커브는 이음매에서 이미 연속이므로 특별 취급이 필요 없다.
